...
const columns: ColumnsType<InventoryList> = [
  ...
  },
  {
    ...
    render: (quantity, record) => {
      ...
      const countValue = record.counts[0].count || 0 // Obtener el primer count o 0 si no existe
      const difference = Math.abs(quantityValue - countValue) // Diferencia absoluta

      // Determinar el color basado en la diferencia
      let color = ''
      if (difference === 0) {
        color = 'green' // Verde si son iguales
      } else if (difference <= 10) {
        color = 'yellow' // Amarillo si están dentro del rango de 10
      } else {
        color = 'red' // Rojo si la diferencia es mayor a 10
      }
